...
export default function FaqPage() {
  ...
  const categories = useMemo(() => {
    const cats = Array.from(new Set(FAQS_STATIC.map((f) => f.category)))
    return ['all', ...cats]
  }, [])

  const filtered = useMemo(() => {
    const q = search.toLowerCase().trim()
    return FAQS_STATIC.filter((f) => {
      const matchesCat = activeCategory === 'all' || f.category === activeCategory
      const matchesSearch = !q || f.q.toLowerCase().includes(q) || f.a.toLowerCase().includes(q)
      return f.visible && matchesCat && matchesSearch
            ...
            >
              ...
              {activeCategory !== cat && (
                <span className="ml-1.5 font-body font-normal text-gray-300">
                  ({FAQS_STATIC.filter((f) => f.visible && (cat === 'all' || f.category === cat)).length})
                </span>
              )}
            </motion.button>
          ))}
