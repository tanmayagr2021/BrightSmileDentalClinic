...
import type { BeforeAfterCaseRow } from '@/lib/content'

type Category = BeforeAfterCaseRow['category'] | 'all'

const CATEGORY_ORDER: Category[] = ['all', 'smile-makeover', 'orthodontics', 'whitening', 'implants']
...
export default function BeforeAfterSection({
  ...
  const categoryLabels: Record<Category, string> = {
    all: pick(content, 'home.before_after.category_all', 'All Cases'),
    'smile-makeover': pick(content, 'home.before_after.category_smile_makeover', 'Smile Makeover'),
    orthodontics: pick(content, 'home.before_after.category_orthodontics', 'Orthodontics'),
    whitening: pick(content, 'home.before_after.category_whitening', 'Whitening'),
    implants: pick(content, 'home.before_after.category_implants', 'Implants'),
  }
            ...
            {filtered.map((item) => (
              ...
              >
                ...
                <div className="relative h-52 overflow-hidden">
                  ...
                  {/* Premium "coming soon" overlay — only until both photos are uploaded */}
                  {(!item.before_image_url || !item.after_image_url) && (
                    <div className="absolute inset-x-0 bottom-0 flex flex-col items-center justify-center gap-2.5 pb-6 pt-4" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.55) 0%, transparent 100%)' }}>
                      <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] px-3.5 py-2 backdrop-blur-sm">
                        ...
                        <span className="font-heading text-[0.65rem] font-semibold tracking-[0.08em] text-white/85">
                          {pick(content, 'home.before_after.awaiting_consent_label', 'Photos — Awaiting Patient Consent')}
                        </span>
                      </div>
                    </div>
                  )}
